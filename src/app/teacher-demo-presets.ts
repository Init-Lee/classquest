 ...
 * 更新触发：演示档案字段、课时4/5 组员导入链路、或 `createDemoPortfolio` 结构变化时同步修订
 */
...
import { buildLesson5VersionChangeLeaderPackage } from "@/infra/persistence/serializers/continue-package"
...
/** 组员在课时4第1关 / 课时5第2关横幅内：导入前 ↔ 导入后（不改指针） */
export type TeacherMemberImportDrill = "l4-before" | "l4-after" | "l5-before" | "l5-after"
...
/**
 * 在当前页切换组员导入演练态：保持 `pointer` 与除目标字段外的档案不变，仅改 `lesson4` / `lesson5` 相关导入字段
 */
