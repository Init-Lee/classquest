...
更新触发：课时目标、Step 数量、状态字段、Guard、教师讲解策略或快照规则变化时，需要同步更新本文件。
-->
...
模块四拥有独立教师讲解模式，不复用模块三业务代码。教师入口在 `/module/4` 首页，进入后顶部显示讲解横幅，Guard 放行，课时 1 各关可直接浏览。

讲解模式的答案显示规则：

- 选择题、判断题、结构配对等有客观答案的任务默认隐藏答案，由教师点击「显示参考答案 / 显示参考配对」后再讲评。
- 填空、计划、说明类内容可以直接展示示例，因为学生无法简单照抄，且适合课堂示范。
- 保存只写内存讲解档案，不污染学生真实数据。
