...
export default function Step5UpgradeVerify() {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      if (isLesson5Enabled()) {
        navigate("/module/3/lesson/5/step/1")
      } else {
        navigate("/module/3")
      }
    } finally {
      ...
      <div className="space-y-6 max-w-3xl">
        ...
        <Card className="border-emerald-200">
          <CardContent className="py-4 space-y-3">
            ...
            {lesson4.completed ? (
              isLesson5Enabled() ? (
                <Button onClick={() => navigate("/module/3/lesson/5/step/1")} className="w-full gap-2">
                  <ChevronRight className="h-4 w-4" /> 进入课时5
                </Button>
              ) : (
                <Button onClick={() => navigate("/module/3")} className="w-full gap-2">
          ...
          <Card className="border-emerald-200">
            ...
            <CardContent className="space-y-3 pb-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                {lesson4.completed ? (
                  isLesson5Enabled() ? (
                    <Button onClick={() => navigate("/module/3/lesson/5/step/1")} className="gap-2">
                      <ChevronRight className="h-4 w-4" /> 进入课时5
                    </Button>
                  ) : (
                    <Button onClick={() => navigate("/module/3")} className="gap-2">
                      <CheckCircle2 className="h-4 w-4" /> 课时4已完成，返回首页
                    </Button>
                  )
                ) : (
