...
export function TopLessonProgress() {
  ...
  const lessonMatch = location.pathname.match(/^\/module\/3\/lesson\/(\d+)/)
  ...
  const getLessonStatus = (lessonId: number): "completed" | "current" | "available" | "locked" => {
    ...
    // 先按各课 completed 标志判「已通关」，避免指针仍停在上一课最后一关时顶栏不显示绿色
    ...
    if (lessonId === 6 && portfolio.lesson6.completed) return "completed"
      ...
      {LESSON_REGISTRY.map((lesson, index) => {
        ...
        return (
          <div key={lesson.id} className="flex items-center">
            <button
              onClick={() => isClickable && navigate(`/module/3/lesson/${lesson.id}/step/1`)}
