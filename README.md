...
│   └── module-4-ai-info-detective/    # 模块 4：AI 信息辨识员，课时 1-3 已开放
└── shared/                            # 业务无关 UI 与纯工具
...
- **模块 4 · AI 信息辨识员**：课时 1「框架发布与样例拆解」、课时 2「素材搜集与合规初筛」和课时 3「题目卡 V1 制作与解析填写」已开放，提供独立本地档案、继续学习包、阶段快照和教师演示模式；课时 3 含 V1 题卡编辑、Qwen 题卡自检助手（可 mock/HTTP）、课时 2 素材手动同步与 `lesson3-full` 快照。详见 `src/modules/module-4-ai-info-detective/README.md`。
- **V1.5 后端**：已提供健康检查与模块 4 课时 3 题卡自检助手 API（mock / Qwen）；真实提交、审核、试答、统计和画廊导出在模块 4 mock 流程稳定后实现。详见 `backend/README.md`。
...
- 当前主线发布：`v0.7.3`，对应模块 4 课时 3 本地前端流程合并（课时 1-3 可完整闯关、保存进度、导入进度、生成阶段快照，并支持教师讲解模式与题卡自检助手）。
- 上一稳定里程碑：`v0.7.2`，模块 4 课时 2「素材搜集与合规初筛」。
- 更早里程碑：`v0.6.0`，模块 3 六课时全部完成时的单应用形态。
- 模块 4 后续开发：每个课时独立分支推进；每完成一课后进入 patch 版本（如 `v0.7.4` 对应课时 4）。
